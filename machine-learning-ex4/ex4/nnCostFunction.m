...
for i=1:m,
    ...
    ytemp=zeros(num_labels,1);
    ytemp(y(i))=1;
    J=J+(-ytemp'*(log(sigmoid(xtemp*Theta2')))')-(1-ytemp')*(log(1-sigmoid(xtemp*Theta2')))';   %计算每一个example的cost function
end;
J=1/m*J;
temptheta1=Theta1(:,2:end);
temptheta2=Theta2(:,2:end);
J=J+lambda/(2*m)*(sum(sum(temptheta1.*temptheta1))+sum(sum(temptheta2.*temptheta2)));
...
for t=1:m,
  ...
  %#######################################
  dt2temp=(Theta2'*delta3)';
  delta2=dt2temp(2:end).*sigmoidGradient(z2);
  %size(delta2)
